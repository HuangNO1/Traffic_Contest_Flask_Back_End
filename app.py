...
@app.route('/get_way_1_route', methods=['GET'])
def get_way_1_route():
...
@app.route('/get_way_2_route', methods=['GET'])
def get_way_2_route():
    ori = request.args.get('ori')
    des = request.args.get('des')
...
@app.route('/get_way_3_route', methods=['GET'])
def get_way_3_route():
    ori = request.args.get('ori')
    des = request.args.get('des')
...
def bd2gcj(ori):
    # 百度坐标系转换为火星坐标系（暂时未使用）
    key = getkey()
    bd2gcj_change_url = 'https://restapi.amap.com/v3/assistant/coordinate/convert?locations=' + \
        ori+'&coordsys=baidu&key='+key
...
def gcj2bd(ori):
    # 火星坐标系转换为百度坐标系（暂时未使用）
    key = baidukey()
    gcj2bd_change_url = 'http://api.map.baidu.com/geoconv/v1/?coords=' + \
        ori+'&from=3&to=5&ak='+key
...
def gethospital(position, distance):
    # 检索给定范围内的医院急诊室
    key = getkey()
    gethospital_url = 'https://restapi.amap.com/v3/place/around?key='+key + \
        '&location='+position+'&keywords=门诊&radius=' + \
        str(distance)+'&types=090101'
    gethospital_result = request_api(gethospital_url)
    counter = int(gethospital_result['count'])
    page = math.ceil(counter/20)
    result = []
    for i in range(1, page+1):
        gethospital_url_page = 'https://restapi.amap.com/v3/place/around?key='+key + \
            '&location='+position+'&keywords=门诊&radius=' + \
            str(distance)+'&types=090101&page='+str(i)
...
def distancecalu(location1, location2):
    ...
    key = getkey()
    distance_index_url = 'http://restapi.amap.com/v3/distance?key=' + \
        key+'&origins='+location1+'&destination='+location2+'&type=0'
...
def getloc(name):
    ...
    loc_index_url = 'https://restapi.amap.com/v3/place/text?keywords=' + \
        name+'&city='+city+'&key='+key+'&extensions=base'
    loc_index_result = request_api(loc_index_url)
    # print(loc_index_result)
...
def navi(ori, des):
    # 请求高德导航路线规划结果并取得最优解
    key = getkey()
    direct_index_url = 'https://restapi.amap.com/v3/direction/driving?strategy=10&origin=' + \
        ori+'&destination='+des+'&key='+key+'&extensions=base'
...
# 构建最优解paths值
    ...
    min_distance = int(distancecalu(
        oriloc, endpoint[mindurationpos]))+int(min_air_distance)
    polyline = endpoint[mindurationpos]+','+desloc
    air_info = {'action': '放飞无人机', 'instruction': '由此处放飞无人机至终点',
                'distance': min_air_distance, 'duration': minduration, 'polyline': polyline}
...
# 构建次优解paths值
    ...
    second_distance = int(distancecalu(
        oriloc, endpoint[seconddurationpos]))+int(second_air_distance)
    second_polyline = endpoint[seconddurationpos]+','+desloc
    air_info = {'action': '放飞无人机', 'instruction': '由此处放飞无人机至终点',
                'distance': second_air_distance, 'duration': secondduration, 'polyline': second_polyline}
    secondresult.append(air_info)
# 输出最优解及次优解
    way_1_route_final_min = {'origin': {'name': ori, 'location': oriloc}, 'destination': {'name': des, 'location': desloc},
                             'option': 0, 'paths': {'path': 0, 'distance': min_distance, 'duration': minduration, 'steps': minresult}}
    way_1_route_final_second = {'origin': {'name': ori, 'location': oriloc}, 'destination': {'name': des, 'location': desloc},
                                'option': 1, 'paths': {'path': 1, 'distance': second_distance, 'duration': secondduration, 'steps': secondresult}}
# 调试检验输出结果
    # print(minresult)
    # print(secondresult)
...
# 将当前方案按照每一个step的起点做分割并记录起点坐标
    ...
    endpoint = []
    naviresult = []
    firstresult = [float(mainnaviresult['duration']),
...
# 计算每个分割点与分割点周围医院急诊室的空中耗时与到达急诊室后的地面运输时间，并取出最小值及对应的急诊室POIID
    for j in range(0, pointnum):
        ...
        nearbyhospital = [nearbyhospital[i]
                          for i in range(0, hospitalnum) if (i not in delepoint)]
        ...
        if hospitalnum != 0:
            ...
            pointnaviresult = [minduration+durationbycar, nearbyhospital[mindurationpos]
                               ['name'], nearbyhospital[mindurationpos]['location']]
        if hospitalnum == 0:
            pointnaviresult = [
                float(mainnaviresult['duration']), 'NOHOSPITAL', 'NOHOSPITAL']
...
# 构建最优解paths值
    ...
    if naviresult[mindurationpos][1] != 'NOHOSPITAL':
        ...
        air_info = {'action': '放飞无人机', 'instruction': '由此处放飞无人机至' +
                    naviresult[mindurationpos][1], 'distance': min_air_distance, 'duration': min_air_duration, 'polyline': polyline}
        minresult.append(air_info)
        navi_after_air = navi(hosloc, desloc)
        min_distance = int(distancecalu(
            oriloc, endpoint[mindurationpos]))+int(min_air_distance)+int(navi_after_air['distance'])
...
# 构建次优解paths值
    ...
    if naviresult[seconddurationpos][1] != 'NOHOSPITAL':
        ...
        air_info = {'action': '放飞无人机', 'instruction': '由此处放飞无人机至' +
                    naviresult[seconddurationpos][1], 'distance': second_air_distance, 'duration': second_air_duration, 'polyline': polyline}
        secondresult.append(air_info)
        navi_after_air = navi(hosloc, desloc)
        second_distance = int(distancecalu(oriloc, endpoint[seconddurationpos]))+int(
            second_air_distance)+int(navi_after_air['distance'])
        for i in range(0, len(navi_after_air['steps'])):
...
# 输出最优解及次优解
    way_2_route_final_min = {'origin': {'name': ori, 'location': oriloc}, 'destination': {'name': des, 'location': desloc}, 'option': 0, 'hosinfo': {
        'hosname': naviresult[mindurationpos][1], 'hosloc': naviresult[mindurationpos][2]}, 'paths': {'path': 0, 'distance': min_distance, 'duration': minduration, 'steps': minresult}}
    way_2_route_final_second = {'origin': {'name': ori, 'location': oriloc}, 'destination': {'name': des, 'location': desloc}, 'option': 1, 'hosinfo': {
        'hosname': naviresult[seconddurationpos][1], 'hosloc': naviresult[seconddurationpos][2]}, 'paths': {'path': 1, 'distance': second_distance, 'duration': secondduration, 'steps': secondresult}}
# 调试检验输出结果
    # print(minresult)
    # print(secondresult)
...
# 大于10公里，寻找跳板
    if distance > distance_limit:
        ...
        theta_od = math.atan2(
            float(des_pos[0])-float(ori_pos[0]), float(des_pos[1])-float(ori_pos[1]))
        condition = 0
        on_search = oriloc
        point_list = []
        q = []
        ...
        while(condition == 0):
            ...
            for i in range(0, len(hospital_list)):
                next_search.append(
                    [hospital_list[i]['name'], hospital_list[i]['location'], hospital_list[i]['distance']])
            # 清洗距离太近的点
            endpoint = []
            ...
            for i in range(0, len(next_search)):
                ...
                theta_temp = math.atan2(float(next_search_pos[0])-float(
                    on_search_pos[0]), float(next_search_pos[1])-float(on_search_pos[1]))
                q_temp = Decimal(str(alpha))*(Decimal(1)-abs(Decimal(str(theta_temp))-Decimal(str(theta_od)))/Decimal(
                    str(math.pi)))+(Decimal(1)-Decimal(str(alpha)))*Decimal(next_search[i][2])/Decimal(10000)
                q_temp = float(q_temp)
                q.append(q_temp)
            max_q = max(q)
    ...
    way_3_route_final = {'origin': {'name': ori, 'location': oriloc}, 'destination': {'name': des, 'location': desloc},
                         'option': 0, 'paths': {'path': 0, 'distance': min_distance, 'duration': minduration, 'steps': steps}}
